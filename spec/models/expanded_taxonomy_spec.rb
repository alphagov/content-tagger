...
RSpec.describe ExpandedTaxonomy do
  ...
  describe "#build" do
    it "returns a representation of the taxonomy, with both parent and child taxons expanded" do
      ...
      expect(taxonomy.root_node.title).to eq "i-Apples"
      expect(taxonomy.parent_expansion.map(&:title)).to eq([
        "i-Apples",
        "i-Fruits",
        "i-Food",
        "i-Red-Things",
      ...
      expect(taxonomy.child_expansion.map(&:title)).to eq([
        "i-Apples",
        "i-Bramley",
        "i-Cox",
      ])
  ...
  describe "#immediate_parents" do
    it "returns immediate parents of the root node" do
      ...
      expect(taxonomy.immediate_parents.map(&:title)).to eq([
        "i-Fruits",
        "i-Red-Things",
      ])
    end
  ...
  describe "#immediate_children" do
    it "returns immediate children of the root node" do
      ...
      expect(taxonomy.immediate_children.map(&:title)).to eq([
        "i-Bramley",
        "i-Cox",
      ])
    end
  ...
  describe "#child_expansion" do
    ...
    context "when the expansion has been built" do
      it "returns the expansion" do
        ...
        expect(taxonomy.child_expansion.map(&:title)).to eq %w(i-Apples i-Bramley i-Cox)
        expect(taxonomy.child_expansion.map(&:node_depth)).to eq [0, 1, 1]
      end
    end
  ...
  describe "#parent_expansion" do
    ...
    context "when the expansion has been built" do
      it "returns the expansion" do
        ...
        expect(taxonomy.parent_expansion.map(&:title)).to eq %w(i-Apples i-Fruits i-Food i-Red-Things)
        expect(taxonomy.parent_expansion.map(&:node_depth)).to eq [0, 1, 2, 1]
      end
    end
  end
