    ...
    context "given some taxon base paths that are linked to content" do
      before do
        ...
        allow(Services.publishing_api).to receive(:get_linked_items)
          .with("foo-taxon-uuid", link_type: 'taxons', fields: %w(content_id base_path))
      ...
      it "deletes the taxon links of any content items tagged to the provided taxons" do
        expect(Services.publishing_api).to receive(:patch_links).with("linked-item-1", links: { taxons: [] })
        expect(Services.publishing_api).to receive(:patch_links).with("linked-item-2", links: { taxons: [] })
        expect(Services.publishing_api).to receive(:patch_links).with("linked-item-3", links: { taxons: [] })
