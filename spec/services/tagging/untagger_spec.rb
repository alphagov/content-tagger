...
RSpec.describe Tagging::Untagger do
  ...
  let(:content_id) { "51ac4247-fd92-470a-a207-6b852a97f2db" }
  ...
  it "untags a taxon" do
    ...
    described_class.call(content_id, %w[aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa])
    assert_publishing_api_patch_links(content_id, "previous_version" => 5, "links" => { "taxons" => %w[bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb] })
  end

  it "retries 3 times" do
    stub_publishing_api_has_links(content_id:, links: { taxons: %w[aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa] }, version: 5)

    stub_any_publishing_api_patch_links.and_raise(GdsApi::HTTPConflict).times(2).then.to_return(body: "{}")
    expect { described_class.call(content_id, %w[aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa]) }.not_to raise_error

    stub_any_publishing_api_patch_links.and_raise(GdsApi::HTTPConflict).times(3).then.to_return(body: "{}")
    expect { described_class.call(content_id, %w[aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa]) }.to raise_error(GdsApi::HTTPConflict)
  end
end
