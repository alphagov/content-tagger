module TaxonomyHealth
  class MaximumDepthMetric
    ...
    def perform(arguments)
      ...
      children_with_depth(taxonomy, maximum_depth).each do |linked_content_item|
  ...
  private
    ...
    def children_with_depth(linked_content_item, maximum_depth, current_depth = 0)
      result = current_depth > maximum_depth ? [linked_content_item] : []
      result + linked_content_item.children.flat_map do |child|
        children_with_depth(child, maximum_depth, current_depth + 1)
      end
    end
  end
end
